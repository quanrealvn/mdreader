...
/// <summary>
/// Find bar over <see cref="IWebViewChannel"/>'s find capability (ARCHITECTURE §4.12): 150 ms debounced restart on
...
public partial class FindBar : UserControl
{
    ...
    {
        InitializeComponent();
        _debounce = new DispatcherTimer(DebounceDelay, DispatcherPriority.Input, OnDebounceTick);
        _debounce.Stop();
        FindTextBox.TextChanged += OnFindTextChanged;
    ...
    public void Open()
    {
        ...
        IsVisible = true;
        FocusTextBox();
        // A bar that was hidden until now may not be focusable before its first layout pass: make sure after it.
        ...
        if (FindTextBox.Text is { Length: > 0 } text && !string.Equals(_activeTerm, text, StringComparison.Ordinal))
        {
            _debounce.Stop();
            _ = RestartAsync();
        }
    ...
    private string Term => FindTextBox.Text ?? "";
    ...
    private void Navigate(bool backwards)
    {
        ...
        if (_debounce.IsEnabled || !string.Equals(_activeTerm, Term, StringComparison.Ordinal) || _session is null)
        {
            _debounce.Stop();
            _ = RestartAsync();
            return;
    ...
    private void OnPreviewKeyDown(object? sender, KeyEventArgs e)
    {
        if (e.Key == Key.Enter && FindTextBox.IsKeyboardFocusWithin)
        {
            ...
            Dispatcher.UIThread.Post(() => Navigate(backwards), DispatcherPriority.Input);
        }
    }
