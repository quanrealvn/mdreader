...
internal sealed class SpikeSession
{
    ...
    private void PostState()
    {
        ...
        if (!_prelude)
        {
            ...
            _prelude = true;
            channel.Post(new ThemeMessage(_theme()));
            channel.Post(new TocVisibilityMessage(true, AppSettings.DefaultTocWidth));
        }
    ...
    private async Task RenderAsync()
    {
        ...
        if (result is DocumentLoadFailed failed)
        {
            ...
            await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() => OnPayloadReady(null, json));
            return;
        }
