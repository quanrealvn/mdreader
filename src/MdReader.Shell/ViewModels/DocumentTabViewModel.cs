...
/// <summary>Services the tab's <see cref="IDocumentTabView"/> needs; the view is created by the shell's document host, not by DI.</summary>
public sealed record DocumentViewServices(
...
/// </summary>
public sealed class DocumentTabViewModel : IDocumentTab
{
    ...
    private IDocumentTabView? _view;
    ...
    public DocumentViewServices ViewServices { get; }

    public IDocumentTabView? View => _view;
    ...
    /// Called once by the tab's view (created by the shell's document host).
    public void AttachView(IDocumentTabView view)
    {
    ...
    /// §4.11 order: find session → DocumentSession (watcher, CTS) → web view.
    public void Dispose()
    {
        if (_disposed)
        {
