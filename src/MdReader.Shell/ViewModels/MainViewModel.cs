...
public sealed class MainViewModel : ObservableObject, ITabHost, IStatusNotifier, IDisposable
{
    ...
    private readonly IUiTimer _statusTimer;
    private readonly IUiTimer _tocButtonResync;
    ...
    public MainViewModel(SettingsCoordinator settings, IThemeService theme, IDialogService dialogs, Lazy<IDocumentOpener> opener,
                         IUiDispatcher dispatcher, IAppLog log)
    {
        _settings = settings;
        _theme = theme;
        _dialogs = dialogs;
        ...
        _statusTimer = _dispatcher.CreateTimer(StatusDuration, OnStatusTimerTick);
        ...
        _tocButtonResync = _dispatcher.CreateTimer(TocButtonResyncDelay, OnTocResyncTick);
    ...
    public void ShowStatus(string message)
    {
        ...
        {
            _dispatcher.Post(() => ShowStatus(message));
            return;
        }
    ...
    private void OnTabPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        if (e.PropertyName is null or nameof(IDocumentTab.Title) or nameof(IDocumentTab.Header))
        {
            ...
            {
                _dispatcher.Post(() => OnPropertyChanged(nameof(WindowTitle)));
            }
        }
        ...
        if (e.PropertyName is null or nameof(IDocumentTab.IsSplitView))
        {
            ...
            {
                _dispatcher.Post(() => OnPropertyChanged(nameof(IsSplitView)));
            }
        }
    }
    ...
    private void OnSettingsChanged(object? sender, AppSettings settings)
    {
        ...
        {
            _dispatcher.Post(() => ApplySettings(_settings.Current));
        }
    }
...
public sealed record RecentFileEntry(string FullPath, string FileName, string Folder)
{
    public static RecentFileEntry Create(string fullPath)
    {
