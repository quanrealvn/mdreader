...
public sealed record DocumentStartInfo(string Path, int DocId, string? Fragment);
...
public sealed class DocumentSession : IDisposable
{
    ...
    public void Detach(IWebViewChannel channel)
    {
        _dispatcher.VerifyAccess();
    ...
    public void OnUserInteractionAfterPrint()
    {
        _dispatcher.VerifyAccess();
    ...
    public bool IsPrintFallbackArmed => _printFallbackArmedAt is not null;
    ...
    {
        _dispatcher.VerifyAccess();
        IWebViewChannel? channel = ReadyChannel();
        if (channel is null)
        {
        ...
        try
        {
            await channel.ShowPrintUiAsync();
    ...
    public async Task<bool> ExportPdfAsync(string pdfPath)
    {
        ...
        try
        {
            bool ok = await channel.PrintToPdfAsync(pdfPath);
            if (ok)
            {
    ...
    public async Task CapturePreviewAsync(Stream pngDestination)
    {
        ...
        IWebViewChannel channel = _channel ?? throw new InvalidOperationException("The document's WebView isn't ready.");
        await channel.CapturePreviewAsync(pngDestination);
    }

    /// The view couldn't show the page at all (WPF error). Waiters fail; ItemStatus becomes error:&lt;kind&gt;.
    public void ReportHostError(DocumentErrorKind kind)
    {
        _dispatcher.VerifyAccess();
        if (_disposed)
        {
    ...
    private async Task PostCurrentPayloadAsync()
    {
        ...
        try
        {
            if (!payload.IsError)
            {
                channel.EnsureResourceRootMapped();
            }
    ...
    private bool LocalResourcesUnavailable => _channel is { LocalResourcesUnavailable: true };
    ...
    private async Task HandleLinkAsync(LinkMessage link)
    {
        try
        {
            ...
            switch (target)
            {
                ...
                case LinkTarget.Blocked blocked:
                    _log.Write(AppLogLevel.Info, Category, $"Blocked link '{LogText.Shorten(link.Href)}': {blocked.Reason}");
        ...
        {
            _log.Write(AppLogLevel.Error, Category, $"Handling the link '{LogText.Shorten(link.Href)}' failed.", ex);
        }
    }
    ...
    private void HandleWebLog(LogMessage message)
    {
        ...
        _log.Write(level, WebCategory, $"[doc {DocId}] {LogText.ForLog(message.Message, MaxLoggedMessageChars)}");
    }
    ...
    private void OnEffectiveThemeChanged(object? sender, EventArgs e)
    {
        ...
        {
            _dispatcher.Post(() => OnEffectiveThemeChanged(sender, e));
            return;
        }
    ...
    private void OnSettingsChanged(object? sender, AppSettings settings)
    {
        if (!_dispatcher.CheckAccess())
        {
            _dispatcher.Post(() => OnSettingsChanged(sender, settings));
            return;
        }
    ...
    private IWebViewChannel? ReadyChannel() => !_disposed && _channel is { IsReady: true } channel ? channel : null;
    ...
    /// Continues on the session's UI thread. Sessions are created by DocumentOpener, which also runs before the UI loop
    /// (CLI files, --capture) when no SynchronizationContext exists yet, so awaits must not rely on the ambient context
    /// to get back to the UI thread.
    private UiYieldAwaitable ReturnToUiThread() => _dispatcher.ReturnToUiThread();
    ...
    private void LogLoadFailure(FailedOutcome failed, bool hasContent)
    {
        ...
        _log.Write(level, Category, $"{phase} {Path} failed: {failed.Error} ({LogText.ForLog(failed.Detail, 500)})");
    }
    ...
    public static string ToProtocolName(DocumentErrorKind kind) => JsonNamingPolicy.CamelCase.ConvertName(kind.ToString());
    ...
    private IUiTimer CreateEditTimer() => _dispatcher.CreateTimer(EditRenderDelay, () =>
    {
        StopEditTimer();
